...
import { ResizeHandle } from '@/components/design/ResizeHandle';
import { ZoomSlider } from '@/components/design/ZoomSlider';
...
const Index = () => {
  ...
  const runExport = useCallback(async (format: 'png' | 'pdf') => {
    ...
    try {
      ...
      const exportWidth = canvasPreset.width;
      const exportHeight = canvasPreset.height;
  ...
  return (
    <div className="app-shell editor-app h-screen flex flex-col bg-background" data-app-shell>
      ...
      <div className="editor-layout flex-1 flex overflow-hidden relative" data-editor-layout>
        ...
        {/* 좌측 패널 토글 — 패널 가장자리에 부착된 반원 탭 */}
        <button
          type="button"
          onClick={() => setLeftCollapsed(v => !v)}
          className="absolute top-1/2 -translate-y-1/2 z-30 h-12 w-5 flex items-center justify-center rounded-r-md border border-l-0 border-border bg-background text-muted-foreground hover:text-foreground hover:bg-accent shadow-sm transition-colors"
          style={{ left: leftCollapsed ? 0 : leftPanelWidth }}
