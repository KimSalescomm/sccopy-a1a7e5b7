...
## 이유 작성 규칙
- 사용자의 문장에서 핵심 표현을 직접 인용하거나 요약하여 근거를 제시하세요.
- 고정 문구를 사용하지 말고, 매번 입력 문장에 따라 다르게 작성하세요.
- 한 줄(30자 이내)로 간결하게 작성하세요.
- 예시: "'벽과 하나 되는 디자인'처럼 의미 중심 표현이 포함됨"
...
## 형식 일관성 규칙 (매우 중요!)
1. 먼저 원문의 형식을 판단하세요:
   - **명사형**: 완결되지 않은 표현 (예: "~디자인", "~기능", "~세탁기")
   - **문장형**: "~합니다", "~해요", "~할 수 있어요" 등 완결 문장
2. 변환 결과는 반드시 원문과 동일한 형식으로 작성하세요.
   - 원문이 명사형이면 → 명사형으로 변환
   - 원문이 문장형이면 → 문장형으로 변환
...
serve(async (req) => {
  ...
  try {
    ...
    if (mode === "convert") {
      ...
      userMessage = `원문: ${text}\n\n목표 유형: ${targetType}\n\n위 카피를 ${targetType} 유형으로 변환해주세요. 원문의 형식(명사형/문장형)을 반드시 유지하세요.`;
    } else {
